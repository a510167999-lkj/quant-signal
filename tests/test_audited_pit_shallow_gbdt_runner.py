...
def test_public_shallow_gbdt_runner_delegates_only_the_frozen_variant(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    ...
    def fake_private_runner(**kwargs: Any) -> dict[str, Any]:
        ...
        return {"schema_version": "synthetic-gbdt-run/v1"}
    ...
    result = ridge.run_audited_pit_ranked_liquidity_shallow_gbdt_rolling_oof(
        settings="settings",
    ...
    assert result == {"schema_version": "synthetic-gbdt-run/v1"}
    assert captured["strategy_spec"] == gbdt.SHALLOW_GBDT_OOF_SPEC
...
def test_public_risk_on_breadth_runner_delegates_only_the_frozen_variant(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    captured: dict[str, Any] = {}
    ...
    def fake_private_runner(**kwargs: Any) -> dict[str, Any]:
        captured.update(kwargs)
        return {"schema_version": "synthetic-risk-on-breadth-run/v1"}
    ...
    result = ridge.run_audited_pit_ranked_liquidity_shallow_gbdt_risk_on_breadth_rolling_oof(
        settings="settings",
        audited_pit_universe_path=tmp_path / "universe.sqlite3",
        expected_coverage_audit_sha256="a" * 64,
        expected_artifact_root_sha256="b" * 64,
    ...
    assert result == {"schema_version": "synthetic-risk-on-breadth-run/v1"}
    assert captured["strategy_spec"] == (
...
def test_jobs_cli_dispatches_frozen_shallow_gbdt_rolling_oof(
    ...
    def fake_runner(**kwargs: Any) -> dict[str, Any]:
        captured.update(kwargs)
        return {"schema_version": "synthetic-gbdt-result/v1"}
...
def test_jobs_cli_dispatches_frozen_risk_on_breadth_rolling_oof(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    from app import jobs
    ...
    def fake_runner(**kwargs: Any) -> dict[str, Any]:
        captured.update(kwargs)
        return {"schema_version": "synthetic-risk-on-breadth-result/v1"}

    monkeypatch.setattr(
        jobs,
        (
            "run_audited_pit_ranked_liquidity_"
            "shallow_gbdt_risk_on_breadth_rolling_oof"
        ),
        fake_runner,
        raising=False,
    )
