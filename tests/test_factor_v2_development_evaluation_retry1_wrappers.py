...
BASE_RUNNER_PATH = (
    RUNS_ROOT / ".run_factor_v2_development_evaluation.py"
)
BASE_RUNNER_SHA256 = (
...
RUNNER_WRAPPER_PATH = (
    ROOT / "scripts" / "run_factor_v2_development_evaluation_retry1.py"
)
VERIFIER_WRAPPER_PATH = (
    ROOT / "scripts" / "verify_factor_v2_development_evaluation_retry1.py"
)
OLD_OUTPUT_DIR = (
    RUNS_ROOT
...
def test_retry_runner_binds_exact_source_and_isolated_paths() -> None:
    ...
    assert runner.STATUS_PATH == NEW_OUTPUT_DIR.with_name(
    ...
    assert runner.STATUS_PATH != OLD_OUTPUT_DIR.with_name(
        f"{OLD_OUTPUT_DIR.name}.run.status.json"
    )
...
def test_retry_verifier_binds_retry_runner_and_new_source() -> None:
    ...
    assert Path(verifier.__file__).resolve() == (
        VERIFIER_WRAPPER_PATH.resolve()
    )
    ...
    assert runner.EXPECTED_SOURCE_COMMIT == EXPECTED_SOURCE_COMMIT
    assert runner.OUTPUT_DIR == NEW_OUTPUT_DIR
    assert Path(runner._BASE_MODULE.__file__).resolve() == (
        RUNNER_WRAPPER_PATH.resolve()
    )
...
def test_retry_wrapper_changes_only_declared_base_bindings(
    ...
    changed = {
        ...
        if original[name] != bound.get(name)
    }

    assert changed == set(wrapper.OVERRIDDEN_BASE_GLOBALS)
    assert wrapper._BASE_MODULE.__file__ == str(
...
def test_retry_wrapper_rejects_base_sha_drift_before_loading(
    ...
    tampered.write_bytes(
        wrapper.BASE_MODULE_PATH.read_bytes() + b"\n# drift\n"
    )
...
def test_retry_wrapper_ignores_environment_binding_overrides(
    ...
    assert (
        wrapper._BASE_MODULE.EXPECTED_SOURCE_COMMIT
        == EXPECTED_SOURCE_COMMIT
    )
...
def test_expected_base_files_remain_exact() -> None:
    assert hashlib.sha256(BASE_RUNNER_PATH.read_bytes()).hexdigest() == (
        BASE_RUNNER_SHA256
    )
    assert hashlib.sha256(BASE_VERIFIER_PATH.read_bytes()).hexdigest() == (
        BASE_VERIFIER_SHA256
    )
