...
def _verify_factor_v3_feature_history_collection_authority(
    *,
    ...
    if attestation_verification is not None:
        ...
        from app import (
            factor_v3_feature_history_frozen_source_attestation as frozen,
        )
        ...
        if type(attested_context.get("authority_binding")) is not dict:
            raise ValueError(
                "factor-v3 feature history attestation binding rejected"
            )
    ...
    if attested_context is not None:
        ...
        if (
            frozen._validated_attested_replay_context(
                **attestation_verification
            )
            != attested_context
        ):
            raise ValueError(
                "factor-v3 feature history attestation source drift rejected"
            )
    unsigned = {
    ...
    if attested_context is not None:
        actual_binding = _factor_v3_feature_history_attestation_binding(
            ...
            feature_history_run_spec_sha256=attested_context[
                "feature_history"
            ]["feature_run_spec_sha256"],
            feature_history_run_root=attestation_verification[
        ...
        if actual_binding != attested_context["authority_binding"]:
            raise ValueError(
                "factor-v3 feature history attestation binding mismatch"
            )
        _validated_collection_publication_issuance(
            output_root=collection_publication_output_root,
            publication=publication,
...
def _verify_feature_history_with_attested_producer_binding(
    ...
    **kwargs: Any,
    ...
    with frozen._locked_physical_frozen_source_binding(
        frozen_source_root
    ):
        return _verify_factor_v3_feature_history_collection_authority(
            **kwargs,
