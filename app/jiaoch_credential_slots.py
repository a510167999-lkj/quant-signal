...
class _FactorV3DailyBasicCredentialGeneration:
    ...
    def __new__(cls, *args: Any, **kwargs: Any) -> _FactorV3DailyBasicCredentialGeneration:
        raise TypeError(
            "Jiaoch factor-v3 daily-basic generation must be created by its factory"
        )
    ...
    def __repr__(self) -> str:
        return (
            "JiaochFactorV3DailyBasicCredentialGeneration("
            f"slots={self.describe_slots()!r})"
        )
...
def _credential_for_factor_v3_daily_basic_route(
    ...
    *,
    ...
    if capability is not _FACTOR_V3_DAILY_BASIC_ROUTE_CAPABILITY:
        raise ValueError(
            "Jiaoch factor-v3 daily-basic private route capability rejected"
        )
    ...
    if (
        ...
        or route_id not in _FACTOR_V3_DAILY_BASIC_ROUTES_BY_ID
    ):
...
def _create_factor_v3_daily_basic_generation(
    *,
    ...
    if not callable(credential_resolver):
        raise ValueError(
            "Jiaoch factor-v3 daily-basic credential resolution rejected"
        )
    try:
        credential = _credential(credential_resolver(_POINTS_PRIMARY_SLOT))
    except Exception as exc:
