...
10. **日滚已装**：工作日 **15:45 / 16:30 / 18:30** 任务 `quant-signal-lkj-path-a-reclaim-oos-roll`。有新 K 线就追加缓存、重算连跌反弹日账本并同步 VPS；没新数据就记还差多少天。不改规则，不放宽 `market_level`，非正式 final-OOS，不注册生产 profile，不自动交易。
...
28. **每日持仓袖**（另开，不是连跌反弹）：`personal_daily_negext_m2`。每个交易日按 −20 日收益取 Top 2，各 ≤30% 本金。不进 Path A 双过，不宣称有效。不要与第 27 条同时打满本金。见 `假设/每日满仓.md`。
