#!/usr/bin/env python3
"""Freeze the user-accepted MA20 reclaim hypothesis. Not formal OOS."""

from __future__ import annotations

import argparse
import os
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from app.factor_v3_path_a_protocol_reclaim_freeze import (  # noqa: E402
    DEFAULT_OUTPUT_ROOT,
    STAGE_GOAL_ID,
    build_path_a_protocol_reclaim_freeze,
    format_reclaim_freeze_table,
    write_path_a_protocol_reclaim_freeze,
)
from app.factor_v3_path_a_protocol_signal import (  # noqa: E402
    DEFAULT_HOLD_QT_PATH,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=STAGE_GOAL_ID)
    parser.add_argument("--repo-root", type=Path, default=ROOT)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument(
        "--qualified-trades-path",
        type=Path,
        default=DEFAULT_HOLD_QT_PATH,
    )
    parser.add_argument("--allow-any-role", action="store_true")
    args = parser.parse_args(argv)
    repo = args.repo_root.resolve()
    os.environ.setdefault("VPS_RUNTIME_ROLE", "local_research")
    qt = args.qualified_trades_path
    report = build_path_a_protocol_reclaim_freeze(
        repo_root=repo,
        qualified_trades_path=qt if qt.is_absolute() else (repo / qt),
        require_local_research=not args.allow_any_role,
    )
    output_root = (
        args.output_root if args.output_root.is_absolute() else (repo / args.output_root)
    ).resolve()
    pointer = write_path_a_protocol_reclaim_freeze(report, output_root=output_root)
    print(format_reclaim_freeze_table(report), end="")
    print("wrote", pointer.get("card_path"))
    return 0 if pointer.get("ok") is True else 2


if __name__ == "__main__":
    raise SystemExit(main())
